...
real-time. It classifies traffic into two categories: 
...
if st.button("Analyze Packet"): 
    if 'model' in st.session_state: 
        model = st.session_state['model'] 
        # Feature order must match training: Port, Duration, Packets, Length, Active 
        ...
        if pred[0] == 1: 
            ...
            st.write("**Analysis Result:** The AI identified patterns matching a network attack.") 
        else: 
